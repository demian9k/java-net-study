package io.demian.net_study.step3_NIO_NONBLOCK.hjhistory;
...
public class SimpleChatClient {
    ...
    class MyThread extends Thread {
        ...
        public void run() {
            ByteBuffer buffer = ByteBuffer.allocateDirect(1024);
            info("mythread run start");
            try {
                while (!Thread.currentThread().isInterrupted()) {
                    info("mythread running 161");
                    buffer.clear();
                    BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
                    info("mythread running 164");
                    String message = in.readLine();
                    info("mythread running " + message);
                    ...
                    buffer.flip();
                    info("mythread running 174");
                    sc.write(buffer);
                    info("mythread running 176");
                }
            } catch (Exception e) {
...
//출처: https://hjhistory.tistory.com/entry/NIO-셀렉터-부분-정리 [My History: 개인 블로그]